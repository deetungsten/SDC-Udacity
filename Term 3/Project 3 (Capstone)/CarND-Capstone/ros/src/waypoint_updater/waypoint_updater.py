...
class WaypointUpdater(object):
    ...
    def get_closest_waypoint_index(self):
        ...
        closest_coodinate = self.waypoints_2d[closest_index]
        prev_coordinate = self.waypoints_2d[closest_index - 1]

        cl_vector = np.array(closest_coodinate)
        prev_vector = np.array(prev_coordinate)
        pos_vector = np.array([x,y])

        resultant = np.dot(cl_vector-prev_vector,pos_vector-cl_vector)
    ...
    def generate_lane(self):
        ...
        base_waypoints = self.base_lane.waypoints[closest_index:furthest_index]

        if self.stopline_wp_idx == -1 or (self.stopline_wp_idx >= furthest_index):
